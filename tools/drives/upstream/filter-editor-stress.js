// Twelve blank conditions on the root, which is the tallest the tree gets.
//
//   cd ~/dev/sg-widgets && pnpm qa --start --path /widgets/filter-editor/ --framework react \
//     --drive ~/dev/sg-widgets-qt/tools/drives/upstream/filter-editor-stress.js \
//     --shot /tmp/ref/filter-editor-stress.png
//
// The Qt half is `QA_STATE=stress tools/drives/states/filter-editor.py`.
function press(el) {
...
const groups = () => $$('[data-slot="filter-group"]', pane);
...
await until(() => (rows().length > 0 ? rows() : null), 'the demo tree');
...
for (let i = 0; i < 12; i += 1) {
  ...
  await until(() => rows().length > held || null, `row ${i + 1}`);
}

$('[data-sg-demo]')?.scrollIntoView({ block: 'start' });
await wait(500);
return { verdict: `PASS stress`, state: 'stress', rows: rows().length, groups: groups().length };
