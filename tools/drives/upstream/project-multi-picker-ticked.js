// The list open on a picker already holding every project, so every row is ticked.
function press(el) {
...
const held = $("[data-slot$=\"-control\"]", box) ?? control;
held.scrollIntoView({ block: "start" });
await wait(300);
press(held);
...
return { verdict: "PASS ticked", rows: rows.length, ticked: rows.filter((r) => r.dataset.selectedEntity === "true").length };
