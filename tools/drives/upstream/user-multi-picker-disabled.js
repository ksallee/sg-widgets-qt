// Disabled: the control at half opacity and inert.
//
//   cd ~/dev/sg-widgets && node tools/qa.mjs --base http://127.0.0.1:4466 \
//       --path /widgets/user-multi-picker/ --framework react \
//       --drive ~/dev/sg-widgets-qt/tools/drives/upstream/user-multi-picker-disabled.js --shot out.png
//
// The Qt half is `tools/drives/user-multi-picker-disabled.py`.
...
// The states section draws one control disabled. The body scrolls to it and answers what it keeps.
...
const found = boxes[3];
if (!found) return { verdict: 'FAIL the states section has no disabled control' };
bring(found, 200);
await wait(500);
return { verdict: 'PASS disabled', read: inertRead(found) };
