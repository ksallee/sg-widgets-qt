// A group added at the root and removed again, so the root is back to its rows.
//
//   cd ~/dev/sg-widgets && pnpm qa --start --path /widgets/filter-editor/ --framework react \
//     --drive ~/dev/sg-widgets-qt/tools/drives/upstream/filter-editor-unnested.js \
//     --shot /tmp/ref/filter-editor-unnested.png
//
// The Qt half is `QA_STATE=unnested tools/drives/states/filter-editor.py`.
function press(el) {
...
await wait(200);
const added = groups().at(-1);
press($('[data-slot="filter-group-header"] [data-slot="filter-remove"]', added));
await until(() => groups().length === groupsBefore || null, 'the group to go');

$('[data-sg-demo]')?.scrollIntoView({ block: 'start' });
await wait(500);
return { verdict: `PASS unnested`, state: 'unnested', rows: rows().length, groups: groups().length };
