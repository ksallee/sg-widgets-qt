// A narrow summary control: whole chips, then `+n`, and the list open behind it.
//
//   cd ~/dev/sg-widgets && node tools/qa.mjs --base http://127.0.0.1:4466 \
//       --path /widgets/user-multi-picker/ --framework react \
//       --drive ~/dev/sg-widgets-qt/tools/drives/upstream/user-multi-picker-overflow.js --shot out.png
//
// The Qt half is `tools/drives/user-multi-picker-overflow.py`.
...
const box = await until(() => summaryBox('ellipsis-narrow'), 15000);
if (!box) return { verdict: 'FAIL no narrow summary control' };
bring(box);
await wait(500);
const control = controlIn(box);
const chips = $$('[data-chip]', control);
const pill = [...control.querySelectorAll('*')].find((el) => /^\+\d+$/.test(el.textContent.trim()));
const count = pill ? Number(pill.textContent.trim().replace('+', '')) : 0;
press(pill ?? $('[data-slot$="-trigger"]', control) ?? control);
if (!(await until(popup))) return { verdict: 'FAIL a press on the +n pill opened no list' };
await until(() => (options().length ? options() : null));
await wait(900);
return {
  verdict: count ? 'PASS whole chips, the rest counted, the list open behind the pill' : 'FAIL nothing was counted',
  chips: chips.length,
  pill: count,
  rows: options().length,
  search_row: Boolean(($('[data-picker="entity"] [data-slot$="-input"]') ?? $('[data-picker="entity-multi"] [data-slot$="-input"]'))),
};
