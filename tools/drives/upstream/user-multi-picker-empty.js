...
//       --drive ~/dev/sg-widgets-qt/tools/drives/upstream/user-multi-picker-empty.js --shot out.png
//
// The Qt half is `tools/drives/user-multi-picker-empty.py`.
...
if (!input) return { verdict: 'FAIL no control in the multi case' };
bring(box);
await wait(400);
openControl(box);
if (!(await until(popup))) return { verdict: 'FAIL the list did not open' };
const caret = caretOf(input);
typeInto(caret, 'zzzqqq');
const line = await until(() => ($('[data-picker="entity"] [data-slot$="-empty"]') ?? $('[data-picker="entity-multi"] [data-slot$="-empty"]')));
await wait(600);
return {
  verdict: line ? 'PASS the empty line stands' : 'FAIL nothing stood for no match',
