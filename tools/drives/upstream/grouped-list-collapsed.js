// The upstream half of `grouped-list` in the `collapsed` state, for the Qt half in
// `tools/drives/states/grouped-list.py`.
//
//   pnpm qa --start --path /widgets/grouped-list/ --framework react \
//     --drive tools/drives/upstream/grouped-list-collapsed.js --shot out.png
...
await wait(900);
frame(box('pages'));
const before = rows('pages').length;
const first = groups('pages')[0];
const held = first.querySelectorAll('[data-slot="grouped-list-row"]').length;
first.querySelector('button').click();
if (!(await until(() => rows('pages').length === before - held))) {
  return { verdict: `FAIL collapsing left ${rows('pages').length} rows`, notes };
}
await wait(400);
return { verdict: 'PASS collapsed', notes: [`${before} -> ${rows('pages').length} rows`] };
