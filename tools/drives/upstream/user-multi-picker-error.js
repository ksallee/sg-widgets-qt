// A read that failed, which this page cannot arm: the rule is read instead.
//
//   cd ~/dev/sg-widgets && node tools/qa.mjs --base http://127.0.0.1:4466 \
//       --path /widgets/user-multi-picker/ --framework react \
//       --drive ~/dev/sg-widgets-qt/tools/drives/upstream/user-multi-picker-error.js --shot out.png
//
// The Qt half is `tools/drives/user-multi-picker-error.py`.
...
// A failed read is not reachable from inside the page: no demo here arms one, upstream or in
// the Qt showcase, and the block is the picker base's. The body answers the destructive token
// the line is drawn in, and `upstream/README.md` says why a state can be read rather than shot.
const box = await until(() => caseBox('multi'), 15000);
bring(box);
await wait(400);
press($('[data-slot$="-input"]', box) ?? controlIn(box));
await until(popup);
await wait(400);
return {
  verdict: 'PASS the error line is the base\'s and no demo on this page arms a failure',
  reachable: false,
  tokens: tokens(),
};
