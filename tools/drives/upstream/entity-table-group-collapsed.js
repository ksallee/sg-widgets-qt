// The upstream half of `entity-table` in the `group-collapsed` state, for the Qt half in
// `tools/drives/states/entity-table.py`.
//
//   pnpm qa --start --path /widgets/entity-table/ --framework react \
//     --drive tools/drives/upstream/entity-table-group-collapsed.js --shot out.png
...
if (!(await until(() => bodyRows().length < before))) return { verdict: 'FAIL the heading did not shut', notes };
frame();
await wait(400);
return { verdict: 'PASS group-collapsed', notes: [`${before} -> ${bodyRows().length} rows`] };
