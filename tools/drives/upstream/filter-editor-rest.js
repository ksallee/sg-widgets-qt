// The seeded tree as it settles: every value control the demo draws.
//
//   cd ~/dev/sg-widgets && pnpm qa --start --path /widgets/filter-editor/ --framework react \
//     --drive ~/dev/sg-widgets-qt/tools/drives/upstream/filter-editor-rest.js \
//     --shot /tmp/ref/filter-editor-rest.png
//
// The Qt half is `QA_STATE=rest tools/drives/states/filter-editor.py`.
function press(el) {
...
await wait(500);
return { verdict: `PASS rest`, state: 'rest', rows: rows().length, groups: groups().length };
