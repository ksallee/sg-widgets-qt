...
type(input, 'd');
await until(() => $$(`[data-slot="${slot}-option"]`).filter((e) => e.getClientRects().length > 0).length > 0, 4000);
await wait(700);
const rows = $$(`[data-slot="${slot}-option"]`).filter((e) => e.getClientRects().length > 0);
return {
  verdict: rows.length ? 'PASS query with bold runs' : 'FAIL the query left no rows',
  slot, query: input.value, rows: rows.length,
  listed: rows.map((e) => e.dataset.option),
  bold: $$('[data-picker] .font-semibold, [data-picker] mark, [data-picker] strong').map((e) => e.textContent),
  search_row: Boolean(input),
};
