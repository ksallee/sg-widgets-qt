// The control holding focus, with the ring rule and whether `:focus-visible` matched.
//
//   cd ~/dev/sg-widgets && node tools/qa.mjs --base http://127.0.0.1:4466 \
//       --path /widgets/user-multi-picker/ --framework react \
//       --drive ~/dev/sg-widgets-qt/tools/drives/upstream/user-multi-picker-focus.js --shot out.png
//
// The Qt half is `tools/drives/user-multi-picker-focus.py`.
...
if (!box) return { verdict: 'FAIL no multi demo' };
bring(box, 200);
await wait(400);
const control = controlIn(box);
...
return {
  verdict: 'PASS focus',
  ring: control.className.split(/\s+/).filter((c) => c.includes('ring')),
