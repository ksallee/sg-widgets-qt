// The upstream half of `entity-table` in the `scroll` state, for the Qt half in
// `tools/drives/states/entity-table.py`.
//
//   pnpm qa --start --path /widgets/entity-table/ --framework react \
//     --drive tools/drives/upstream/entity-table-scroll.js --shot out.png
...
await wait(900);
press(button('Scroll'));
if (!(await until(() => pane().querySelector('[data-slot="entity-table-sentinel"]')))) {
  return { verdict: 'FAIL no sentinel', notes };
}
if (pane().querySelector('[data-slot="entity-table-pager"]')) return { verdict: 'FAIL the pager stayed up', notes };
frame();
await wait(400);
return { verdict: 'PASS scroll', notes };
