// The first row pointed at First Frame on `is any of`, so the value list stands.
//
//   cd ~/dev/sg-widgets && pnpm qa --start --path /widgets/filter-editor/ --framework react \
//     --drive ~/dev/sg-widgets-qt/tools/drives/upstream/filter-editor-field-list.js \
//     --shot /tmp/ref/filter-editor-field-list.png
//
// The Qt half is `QA_STATE=field-list tools/drives/states/filter-editor.py`.
function press(el) {
...
await pickField(rows()[0], 'First Frame');
await setPreset(rows()[0], 'in');

$('[data-sg-demo]')?.scrollIntoView({ block: 'start' });
await wait(500);
return { verdict: `PASS field-list`, state: 'field-list', rows: rows().length, groups: groups().length };
