// The control holding a keyboard focus ring.
//
//   cd ~/dev/sg-widgets && node tools/qa.mjs --base http://127.0.0.1:4466 \
//     --path /widgets/status-multi-picker/ --framework react \
//     --drive ~/dev/sg-widgets-qt/tools/drives/upstream/status-multi-picker-focus.js --shot /tmp/ref/status-multi-picker-focus.png
//
// The Qt half is `tools/drives/status-multi-picker-focus.py`.
function press(el) {
...
return {
  verdict: 'PASS focus read from the component',
  state: 'focus',
  classes: [control.className, control.tagName, control.getAttribute('tabindex') ?? ''],
};
