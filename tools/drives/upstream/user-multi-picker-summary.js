...
//       --drive ~/dev/sg-widgets-qt/tools/drives/upstream/user-multi-picker-summary.js --shot out.png
//
// The Qt half is `tools/drives/user-multi-picker-summary.py`.
...
const section = await until(() => caseBox('summary'), 15000);
if (!section) return { verdict: 'FAIL no summary section' };
bring(section, 60);
await wait(700);
const read = ['chips', 'chips-narrow', 'ellipsis', 'ellipsis-narrow', 'count', 'count-narrow', 'max']
  .map((name) => {
    const box = summaryBox(name);
    if (!box) return null;
    const control = controlIn(box);
    const pill = $('[data-slot$="-overflow"]', control) ?? [...control.querySelectorAll('*')].find((el) => /^\+\d+$/.test(el.textContent.trim()));
    return {
      demo: name,
      chips: $$('[data-chip]', control).length,
      pill: pill ? Number(pill.textContent.trim().replace('+', '')) : 0,
      text: control.textContent.trim().slice(0, 24),
      w: Math.round(control.getBoundingClientRect().width),
      h: Math.round(control.getBoundingClientRect().height),
    };
  })
  .filter(Boolean);
return { verdict: read.length ? 'PASS the summary modes are on show' : 'FAIL no summary demo found', read };
