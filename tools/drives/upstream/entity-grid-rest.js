// The upstream half of `entity-grid` in the `rest` state, for the Qt half in
// `tools/drives/states/entity-grid.py`.
//
//   pnpm qa --start --path /widgets/entity-grid/ --framework react \
//     --drive tools/drives/upstream/entity-grid-rest.js --shot out.png
...
const badged = tiles('grid-sizes').filter((t) => t.querySelector('[data-slot="entity-card-media"] [data-slot="status-badge"]'));
const variant = badged[0]?.querySelector('[data-slot="status-badge"]')?.getAttribute('data-variant');
notes.push(`${tiles('grid-sizes').length} tiles, ${badged.length} with a status corner of variant ${variant}`);
if (badged.length !== tiles('grid-sizes').length) return { verdict: 'FAIL a tile carries no status corner', notes };
await wait(400);
