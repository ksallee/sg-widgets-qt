...
//       --drive ~/dev/sg-widgets-qt/tools/drives/upstream/user-multi-picker-rest.js --shot out.png
//
// The Qt half is `tools/drives/user-multi-picker-rest.py`.
...
await until(() => options().length >= 0 && caseBox('states'), 15000);
await wait(600);
const boxes = $$('[data-slot="entity-picker"]', pane);
return {
  verdict: boxes.length ? 'PASS rest' : 'FAIL no picker on the page',
  controls: boxes.length,
  placeholders: [...new Set($$('[data-slot$="-input"]', pane).map((i) => i.placeholder).filter(Boolean))],
};
