// The upstream half of `entity-table` in the `all-selected` state, for the Qt half in
// `tools/drives/states/entity-table.py`.
//
//   pnpm qa --start --path /widgets/entity-table/ --framework react \
//     --drive tools/drives/upstream/entity-table-all-selected.js --shot out.png
...
// The head box reports the set: `checked` once every loaded row is in it.
const full = () => head.getAttribute('data-state') === 'checked' || head.getAttribute('aria-checked') === 'true';
if (!(await settleCheck(head, full))) {
  return { verdict: `FAIL the head reads ${head.getAttribute('data-state') ?? head.getAttribute('aria-checked')}`, notes };
}
frame();
await wait(400);
return { verdict: 'PASS all-selected', notes: [`${bodyRows().length} rows taken, the head reads ${head.getAttribute('data-state') ?? head.getAttribute('aria-checked')}`] };
