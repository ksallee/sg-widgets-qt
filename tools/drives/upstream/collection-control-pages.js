// The upstream half of `collection-control` in the `pages` state, for the Qt half in
// `tools/drives/states/collection-control.py`.
//
//   pnpm qa --start --path /widgets/collection-control/ --framework react \
//     --drive tools/drives/upstream/collection-control-pages.js --shot out.png
...
await wait(900);
frame();
notes.push(`${rows().length} rows, the footer reads "${pane().textContent.match(/\d+ to \d+ of \d+/)?.[0] ?? ''}"`);
await wait(400);
