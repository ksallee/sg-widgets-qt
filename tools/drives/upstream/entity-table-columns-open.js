// The upstream half of `entity-table` in the `columns-open` state, for the Qt half in
// `tools/drives/states/entity-table.py`.
//
//   pnpm qa --start --path /widgets/entity-table/ --framework react \
//     --drive tools/drives/upstream/entity-table-columns-open.js --shot out.png
...
if (!(await until(() => pane().querySelector('[data-slot="column-picker"]')))) {
  return { verdict: 'FAIL the column picker stayed shut', notes };
}
frame();
await wait(400);
return { verdict: 'PASS columns-open', notes };
