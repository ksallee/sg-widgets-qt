// Invalid: the border and the ring in `destructive`.
//
//   cd ~/dev/sg-widgets && node tools/qa.mjs --base http://127.0.0.1:4466 \
//       --path /widgets/user-multi-picker/ --framework react \
//       --drive ~/dev/sg-widgets-qt/tools/drives/upstream/user-multi-picker-invalid.js --shot out.png
//
// The Qt half is `tools/drives/user-multi-picker-invalid.py`.
...
// The states section draws one control invalid. The body scrolls to it and answers what it keeps.
...
const found = boxes[5];
if (!found) return { verdict: 'FAIL the states section has no invalid control' };
bring(found, 200);
await wait(500);
return { verdict: 'PASS invalid', read: inertRead(found) };
