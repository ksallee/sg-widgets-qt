// The upstream half of `grouped-list` in the `rest` state, for the Qt half in
// `tools/drives/states/grouped-list.py`.
//
//   pnpm qa --start --path /widgets/grouped-list/ --framework react \
//     --drive tools/drives/upstream/grouped-list-rest.js --shot out.png
...
await wait(900);
frame(box('pages'));
const stated = groups('pages').map((g) => Number(g.querySelector('button').lastElementChild.textContent.trim()));
const shown = groups('pages').map((g) => g.querySelectorAll('[data-slot="grouped-list-row"]').length);
notes.push(`counts stated ${stated.join(',')} against ${shown.join(',')} rows`);
if (stated.join(',') !== shown.join(',')) return { verdict: 'FAIL a heading miscounts its rows', notes };
await wait(400);
