// The upstream half of `grouped-list` in the `selected` state, for the Qt half in
// `tools/drives/states/grouped-list.py`.
//
//   pnpm qa --start --path /widgets/grouped-list/ --framework react \
//     --drive tools/drives/upstream/grouped-list-selected.js --shot out.png
...
await wait(900);
frame(box('pages'));
const checks = rows('pages').map((r) => r.querySelector('[data-slot="checkbox"]'));
checks[0].click();
checks[1].click();
const count = await until(() => {
  const line = pane().querySelector('[data-testid="selection-count"]')?.textContent.trim();
  return line && line.startsWith('2 ') ? line : null;
});
if (!count) return { verdict: 'FAIL the two rows were not taken', notes };
await wait(400);
