// No demo on this page is readonly: it is the caller's prop. The body answers the classes the
// readonly rule carries, so the affordances the Qt twin drops are compared against the rule.
function press(el) {
...
return {
  verdict: 'PASS readonly is the caller\'s prop and this page never sets it',
  reachable: false,
  readonly_rules: control.className.split(/\s+/).filter((c) => c.includes('readonly') || c.includes('read-only')),
