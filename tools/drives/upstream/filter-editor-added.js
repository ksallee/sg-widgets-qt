// A blank condition added at the root, with its issue line under it.
//
//   cd ~/dev/sg-widgets && pnpm qa --start --path /widgets/filter-editor/ --framework react \
//     --drive ~/dev/sg-widgets-qt/tools/drives/upstream/filter-editor-added.js \
//     --shot /tmp/ref/filter-editor-added.png
//
// The Qt half is `QA_STATE=added tools/drives/states/filter-editor.py`.
function press(el) {
...
press($('[data-slot="filter-add-condition"][data-path=""]', editor()));
await until(() => rows().length > before || null, 'the new row');

$('[data-sg-demo]')?.scrollIntoView({ block: 'start' });
await wait(500);
return { verdict: `PASS added`, state: 'added', rows: rows().length, groups: groups().length };
