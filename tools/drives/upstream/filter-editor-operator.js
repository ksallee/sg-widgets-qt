// The first row moved onto `is none of`, so the value control swaps.
//
//   cd ~/dev/sg-widgets && pnpm qa --start --path /widgets/filter-editor/ --framework react \
//     --drive ~/dev/sg-widgets-qt/tools/drives/upstream/filter-editor-operator.js \
//     --shot /tmp/ref/filter-editor-operator.png
//
// The Qt half is `QA_STATE=operator tools/drives/states/filter-editor.py`.
function press(el) {
...
await setPreset(rows()[0], 'not_in');

$('[data-sg-demo]')?.scrollIntoView({ block: 'start' });
await wait(500);
return { verdict: `PASS operator`, state: 'operator', rows: rows().length, groups: groups().length };
