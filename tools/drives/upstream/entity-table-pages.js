// The upstream half of `entity-table` in the `pages` state, for the Qt half in
// `tools/drives/states/entity-table.py`.
//
//   pnpm qa --start --path /widgets/entity-table/ --framework react \
//     --drive tools/drives/upstream/entity-table-pages.js --shot out.png
...
await wait(900);
frame();
notes.push(`the footer reads "${range()}" over ${bodyRows().length} rows`);
if (range() !== '1 to 25 of 320') return { verdict: `FAIL the range reads "${range()}"`, notes };
return { verdict: 'PASS pages', notes };
