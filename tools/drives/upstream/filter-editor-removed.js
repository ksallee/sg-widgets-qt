// The first root condition removed.
//
//   cd ~/dev/sg-widgets && pnpm qa --start --path /widgets/filter-editor/ --framework react \
//     --drive ~/dev/sg-widgets-qt/tools/drives/upstream/filter-editor-removed.js \
//     --shot /tmp/ref/filter-editor-removed.png
//
// The Qt half is `QA_STATE=removed tools/drives/states/filter-editor.py`.
function press(el) {
...
press($('[data-slot="filter-remove"]', rows()[0]));
await until(() => rows().length < before || null, 'the row to go');

$('[data-sg-demo]')?.scrollIntoView({ block: 'start' });
await wait(500);
return { verdict: `PASS removed`, state: 'removed', rows: rows().length, groups: groups().length };
