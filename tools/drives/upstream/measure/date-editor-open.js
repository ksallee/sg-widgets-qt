...
await settle();
// The md date editor with its popover open, the way `tools/drives/upstream/date-editor-open.js`
// opens it, so the surface the port is drawn against is standing when the measure runs.
{
  const row = document.querySelector('[data-demo-row="md"]') ?? document;
  row.scrollIntoView({ block: 'start' });
  await wait(400);
  const trigger = row.querySelector('[data-slot="date-editor-trigger"], [data-slot="date-time-editor-trigger"]');
