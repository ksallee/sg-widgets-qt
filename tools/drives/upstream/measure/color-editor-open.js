...
await settle();
// The colour picker open on the first swatch, so the popover it paints, its inset, its ring and
// the shadow around it have a twin to be read against.
{
  const box = document.querySelectorAll('[data-slot="color-editor"]')[0];
  box.scrollIntoView({ block: 'center' });
  await wait(300);
  const swatch = box.querySelector('[data-slot="color-editor-swatch"]');
  for (const type of ['pointerdown', 'mousedown', 'pointerup', 'mouseup', 'click']) {
    swatch.dispatchEvent(new MouseEvent(type, { bubbles: true, cancelable: true, view: window, button: 0 }));
  }
  await wait(700);
}
