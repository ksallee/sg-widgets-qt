// The first picker control on the page, open, for the popup the port is read against.
//
// Base UI opens a combobox on `pointerdown`, not on `click`, so the press is dispatched the way
// `tools/drives/upstream/entity-chip-preview-open.js` dispatches a hover: the whole sequence.
for (let i = 0; i < 40 && $$('[data-slot="department-picker-trigger"]').length === 0; i += 1) await wait(250);
const trigger = $('[data-slot="department-picker-trigger"]');
if (!trigger) return { verdict: 'FAIL the page drew no picker trigger' };
trigger.scrollIntoView({ block: 'center' });
await wait(200);
const spot = trigger.getBoundingClientRect();
const init = {
...
trigger.click();
for (let i = 0; i < 40 && $$('[data-slot="department-picker-content"]').length === 0; i += 1) await wait(150);
await wait(500);
...
function measurePortals() {
  const roots = [...document.querySelectorAll(
    ...
    // A picker's popup is `<slot>-content`, and a menu or a select puts its own list outside the
    // pane too, so anything the page portals out is caught by its position rather than its name.
    '[data-slot$="-content"], [data-slot$="-popup"], [data-popup-open], [data-open]'
  )].filter((el) => !el.closest('[data-pane="react"]'));
  const wanted = [
