// Read-only: full contrast, no chevron and no clear control.
//
//   cd ~/dev/sg-widgets && node tools/qa.mjs --base http://127.0.0.1:4466 \
//       --path /widgets/user-multi-picker/ --framework react \
//       --drive ~/dev/sg-widgets-qt/tools/drives/upstream/user-multi-picker-readonly.js --shot out.png
//
// The Qt half is `tools/drives/user-multi-picker-readonly.py`.
...
// The states section draws one control readonly. The body scrolls to it and answers what it keeps.
const section = await until(() => caseBox('states'), 15000);
if (!section) return { verdict: 'FAIL no states section' };
const boxes = $$('[data-slot="entity-picker"]', section);
const found = boxes[4];
if (!found) return { verdict: 'FAIL the states section has no readonly control' };
bring(found, 200);
await wait(500);
return { verdict: 'PASS readonly', read: inertRead(found) };
