// The upstream half of `entity-table` in the `compact` state, for the Qt half in
// `tools/drives/states/entity-table.py`.
//
//   pnpm qa --start --path /widgets/entity-table/ --framework react \
//     --drive tools/drives/upstream/entity-table-compact.js --shot out.png
...
await wait(900);
const tall = bodyRows()[0].getBoundingClientRect().height;
press(button('Compact'));
await wait(500);
const short = bodyRows()[0].getBoundingClientRect().height;
