// The first row pointed at Date Created on `is`, so a date editor stands.
//
//   cd ~/dev/sg-widgets && pnpm qa --start --path /widgets/filter-editor/ --framework react \
//     --drive ~/dev/sg-widgets-qt/tools/drives/upstream/filter-editor-field-date.js \
//     --shot /tmp/ref/filter-editor-field-date.png
//
// The Qt half is `QA_STATE=field-date tools/drives/states/filter-editor.py`.
function press(el) {
...
await pickField(rows()[0], 'Date Created');
await setPreset(rows()[0], 'is');

$('[data-sg-demo]')?.scrollIntoView({ block: 'start' });
await wait(500);
return { verdict: `PASS field-date`, state: 'field-date', rows: rows().length, groups: groups().length };
