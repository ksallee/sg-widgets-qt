...
| sort-picker | complete | `sort-picker.tsx`, `sort-picker.svelte`, `sort-picker.ts`, `sort-picker.mdx`, `Demo.tsx`, `sortable.tsx` | The rows in that order are entity-table, as upstream draws them. options hands the field picker a flat list and drops the chosen keys from it itself, as upstream's flat does; paths keeps its meaning and still nests. |
...
| entity-table | complete | `Demo.tsx` | The toolbar's sort control takes the shown columns through options, so it sorts on the linked column too. |
| entity-table-infinite | complete | `Demo.tsx` |  |
| entity-tree | complete | `Demo.tsx` |  |
| filter-editor | complete | `Demo.tsx` | The catalogue tree is seeded row for row: six groups, forty rows, the same field paths, operators and value shapes. A link on many values draws EntityMultiPicker, which is the Qt widget behind the web's multiple entity picker; a range and a list of values are the editor's own filter-range and filter-list. The live branch is the status row alone, as upstream. |
...
| filter-editor | complete | `filter-editor.mdx`, `entity-fields.ts`, `filter-editor.ts` | Qt notes written by the widget pass. The demo caption and the catalogue paragraph come from that branch. |
